...
public function makepayment(Request $request){
   ...
   $txref = "rave-29933830"; // ensure you generate unique references per transaction.
   $PBFPubKey = env('RAVE_SID'); // get your public key from the dashboard.
   $redirect_url = "http://127.0.0.1/api/pays";
    $data = [
    'customer_email' => $customer_email,
    'amount' => $amount,  
    'currency' => $currency,
    'txref' => $txref, // ensure you generate unique references per transaction.
    'PBFPubKey' => $PBFPubKey, // get your public key from the dashboard.
    'redirect_url' => $redirect_url
    ];
    $client = new Client(['verify' => false], ['timeout'  => 60]);

    $result = $client->request('POST','https://ravesandboxapi.flutterwave.com/flwv3-pug/getpaidx/api/v2/hosted/pay', [
        'json' => $data,
        'headers' => [
     ...
     $transaction = json_decode($result->getBody());
